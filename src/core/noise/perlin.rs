...
fn get_constant_vector(hash: usize) -> Vector2f {
    let h = hash & 3;

    match h {
        0 => Vector2f::new(1.0, 1.0),
        1 => Vector2f::new(-1.0, 1.0),
        2 => Vector2f::new(-1.0, -1.0),
        _ => Vector2f::new(1.0, -1.0),
    }
}
...
pub fn perlin2d(x: f32, y: f32, perm: &PermutationTable) -> f32 {
    ...
    let top_right = Vector2f::new(xf - 1.0, yf - 1.0);
    let top_left = Vector2f::new(xf, yf - 1.0);
    let bottom_right = Vector2f::new(xf - 1.0, yf);
    let bottom_left = Vector2f::new(xf, yf);
