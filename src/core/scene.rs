...
use crate::event::{CustomGameEvent, GameEvent};
...
/// to pop the stack.
pub struct SceneStack<GE>
where
    GE: CustomGameEvent,
{
    states: Vec<Box<dyn Scene<GE>>>,
}

impl<GE> Default for SceneStack<GE>
where
    GE: CustomGameEvent,
{
    fn default() -> Self {
...
pub enum SceneResult<GE>
where
    GE: CustomGameEvent,
{
    ReplaceScene(Box<dyn Scene<GE>>),
    Push(Box<dyn Scene<GE>>),
    Pop,
    /// Remove all existing scenes and create the new one.
    ReplaceAll(Box<dyn Scene<GE>>),
    Noop,
}

impl<GE> SceneStack<GE>
where
    GE: CustomGameEvent,
{
    pub fn apply_result(
    ...
    pub fn pop(&mut self, world: &mut hecs::World) -> Option<Box<dyn Scene<GE>>> {
    ...
    pub fn current_mut(&mut self) -> Option<&mut Box<dyn Scene<GE>>> {
    ...
    GE: CustomGameEvent,
{
    ...
    fn update(&mut self, dt: Duration, world: &mut World, resources: &Resources)
        -> SceneResult<GE>;
    ...
    fn process_event(&mut self, _world: &mut World, _ev: GameEvent<GE>, _resources: &Resources) {}
