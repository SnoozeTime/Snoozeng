    ...
    A: InputAction + 'static,
{
    ...
    pub fn with_physic_config(mut self, physic_config: PhysicConfiguration) -> Self {
        self.physic_config = Some(physic_config);
        self
    }
    ...
    pub fn build(mut self, surface: &mut Context) -> Game<A> {
        ...
        if let Some(c) = self.physic_config {
            self.resources.insert(CollisionWorld::new(c));
        } else {
            self.resources.insert(CollisionWorld::default());
        }
